...
namespace EmployeeClient.Controllers
{
	public class HeaderInvoiceController : Controller
	{
		...
		public IActionResult Create()
		{
			...
			ViewBag.ProductCode = productService.GetAllProducts().ConvertAll(x => new SelectListItem{ Text = x.ProductCode, Value = x.ProductId.ToString() }).ToList();
			ViewBag.ProductVat = productService.GetAllProducts().ConvertAll(x => new SelectListItem { Text = x.ProductVat.ToString(), Value = x.ProductId.ToString()}).ToList();
			ViewBag.UnitPrice = productService.GetAllProducts().ConvertAll(x => new SelectListItem { Text = x.ProductBuyingPrice.ToString(), Value = x.ProductId.ToString() }).ToList();
			return View(header); 
		}
		[HttpPost]
