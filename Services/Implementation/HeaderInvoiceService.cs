...
namespace EmployeeClient.Services.Implementation
{
    public class HeaderInvoiceService : IHeaderInvoiceService
    {
        ...
        public GoodReceivedHeader CreateGood(GoodReceivedHeader header)
        {
            string json = JsonConvert.SerializeObject(header);
            HttpResponseMessage responseMessage = client.PostAsync(url + "/" + header.PInvoiceHeaderId, new StringContent(json, Encoding.UTF8, "application/json")).Result;
            if (responseMessage.IsSuccessStatusCode)
            {
                string response = responseMessage.Content.ReadAsStringAsync().Result;
                var result = JsonConvert.DeserializeObject<GoodReceivedHeader>(response);
                if (result != null) header = result;
            }
            else
            {
                string response = responseMessage.Content.ReadAsStringAsync().Result;
                throw new Exception("Error Occurred at the API End Point" + response);
            }
            return header;
        }
