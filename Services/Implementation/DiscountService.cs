...
namespace EmployeeClient.Services.Implementation
{
	...
	{
		private readonly string url = "https://localhost:7243/api/Discount";
		...
		public Discount UpdateDiscount(Discount discount)
		{
			...
			string json = JsonConvert.SerializeObject(discount);
			HttpResponseMessage responseMessage = client.PutAsync(url + "/" + id, new StringContent(json, Encoding.UTF8, "application/json")).Result;
			if(!responseMessage.IsSuccessStatusCode)
			{
				string result = responseMessage.Content.ReadAsStringAsync().Result;
				throw new Exception("Error Occurred at the API End Point!!" +result);
			}
			return discount;
		}
	}
